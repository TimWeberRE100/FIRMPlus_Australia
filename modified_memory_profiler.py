...
def _get_memory(process, backend, timestamps=False, include_children=False, filename=None):
    ...
    def ps_util_tool():
        ...
        try:
            ...
            mem = getattr(process, meminfo_attr)()[1] / _TWO_20
            if include_children:
    ...
    if isinstance(proc, (list, tuple)):
        ...
        while True:
            if not max_usage:
                ...
                ret = max(ret,
                          _get_memory(
                              process, backend, include_children=include_children))
            time.sleep(interval)
            line_count += 1
            ...
            if timeout is not None:
                ...
                if max_iter == 0:
                    break
            procpoll, returncode = proc.poll(), proc.returncode
