...
parser.add_argument(
    "-cb", default=0, type=int, required=False, help="Callback: 0-None, 1-generation elites, 2-everything"
)
...
MLoad = np.genfromtxt(
    "Data/electricity.csv", delimiter=",", skip_header=1, usecols=range(4, 4 + len(Nodel))
)  # EOLoad(t, j), MW
MLoad /= 1000  # MW/GW
...
TSWind = np.genfromtxt(
    "Data/wind.csv", delimiter=",", skip_header=1, usecols=range(4, 4 + len(Windl))
)  # TSWind(t, i), MW
...
lb = np.array([0.0] * pzones + [0.0] * wzones + [0.0] * nodes + [0.0] * nodes + [0.0] * nhvi)
ub = np.array([32.0] * pzones + [32.0] * wzones + [32.0] * nodes + [500.0] * nodes + [100.0] * nhvi)
