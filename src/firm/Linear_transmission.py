...
def build_model(solution):
    ...
    model.constr_charge_power_upper = pyo.Constraint(model.t, model.n, rule=lambda m, t, n: m.charge[t, n] <= solution.CPHP[n-1])
    # model.constr_charge_power_lower = pyo.Constraint(model.t, model.n, rule=lambda m, t, n: m.charge[t, n] >= 0)
    model.constr_discharge_power_upper = pyo.Constraint(model.t, model.n, rule=lambda m, t, n: m.discharge[t, n] <= solution.CPHP[n-1])
    # model.constr_discharge_power_lower = pyo.Constraint(model.t, model.n, rule=lambda m, t, n: m.discharge[t, n] >= 0)
    model.constr_storage_energy_upper = pyo.Constraint(model.t, model.n, rule=lambda m, t, n: m.storage[t, n] <= solution.CPHS[n-1])
    ...
    model.constr_hvdc_power_import = pyo.Constraint(model.t, model.l, rule=lambda m, t, l: m.hvdc_pos[t, l] <= solution.CHVDC[l-1])
    model.constr_hvdc_power_export = pyo.Constraint(model.t, model.l, rule=lambda m, t, l: m.hvdc_neg[t, l] <= solution.CHVDC[l-1])
    ...
    def expr_energy_balance(m, t, n):
        return (solution.MLoad[t-1, n-1].sum()
                + m.charge[t, n] 
